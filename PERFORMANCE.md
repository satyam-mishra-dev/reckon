...
_Safety:_ verified the account IDs are stable across a chaos truncate+reseed
...
_Safety (unchanged invariants):_ a crash mid-merged-TX rolls back to
...
This is the measurement that is _not_ subject to machine noise: the pipeline
...
|   64 | 121.7 → 116.7 ms |  −4% | 172.7 → 215.0 | 493 → 411    | −17% _(noise)_ |
...
- **Eliminate the per-phase `loadKey`/`loadIntent` re-reads** (≈4 more
  round-trips). The resume loop deliberately _re-derives all state from the DB_ on
...
- **Merge `provider_charged` into the finish TX too** (removing that recovery
  point). Correct _only if_ the provider is idempotent on the derived key — but
...
The create path is still **round-trip-bound**: ~26 _sequential_ API→Postgres
...
1. **Fuse each phase's writes into one server round-trip.** Today a phase is
   ...
   is the single biggest remaining win, _without_ touching the re-derive contract
...
2. **Measure on a dedicated host.** Re-run baseline vs optimized on a box that is
   _not_ running two other stacks, to get a clean read and to re-test the pool-size
